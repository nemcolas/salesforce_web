 ...
 */
async function getAuthTokens(request: NextResponse) {
  const ttsManager = new IamTokenManager({
    iamApikey: TTS_CREDENTIALS.apikey,
  });
  console.log(ttsManager)
  ...
  return Response.json({ ttsToken, ttsURL: TTS_CREDENTIALS.url, sttToken, sttURL: STT_CREDENTIALS.url });
}

/**
 * Reads the API key from the given file.
 */
function loadCredentials(fileName: any) {
